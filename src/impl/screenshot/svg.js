...
function sImpl_number(
	...
	if (sImpl_take == 2) {
		sImpl_svgData = sImpl_svgData.concat("\t<text x=\"" + x + "\" y=\"" + (y + fontSize) + "\" fill=\"#" + colorHex + "\">" + number + "</text>\n");
	}
	return;
}
...
function sImpl_start(impl) {
	if (sImpl_take == 1) {
		...
		sImpl_svgData = sImpl_svgData.concat("\t<!-- " + programName.string + " v" + programVersion.string + " -->\n");
		sImpl_svgData = sImpl_svgData.concat("\t<!-- Implementation: " + impl + " -->\n");
		sImpl_svgData = sImpl_svgData.concat("\t<!-- Screenshot Implementation: SVG (C-WASM) -->\n");
		...
		sImpl_svgData = sImpl_svgData.concat("\t\t\tfont-size: " + fontSize + "px;\n");
