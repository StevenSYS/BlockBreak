...
function sImpl_number(
	...
	if (sImpl_take == 2) {
		sImpl_svgData += (`\t<text x="${x}" y="${y + FONT_HEIGHT}" fill="#${colorHex}">${number}</text>\n`);
	}
	return;
}
...
function sImpl_start(impl) {
	if (sImpl_take == 1) {
		...
		sImpl_svgData += (`\t<!-- ${PROGRAM_NAME} -->\n`);
		sImpl_svgData += (`\t<!-- Implementation: ${impl} -->\n`);
		sImpl_svgData += (`\t<!-- Screenshot Implementation: SVG (JS) -->\n`);
		sImpl_svgData += (`\t<style>\n`);
		sImpl_svgData += (`\t\ttext {\n`);
		sImpl_svgData += (`\t\t\tfont-family: sans-serif;\n`);
		sImpl_svgData += (`\t\t\tfont-size: ${FONT_HEIGHT}px;\n`);
		sImpl_svgData += (`\t\t}\n`);
		sImpl_svgData += (`\t</style>\n`);
